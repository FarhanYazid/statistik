<x-template-layout>
    <div class="content w-full">
            ...
            <div class="w-full text-center mt-2 ml-1 col-span-4">
                <form action="{{route('data.store')}}" method="POST">
                    ...
                    <div class="max-w-sm mx-auto p-1 pr-0 flex items-start form-group">
                        <input type="number" name="skor" for="skor" placeholder="Masukkan Skor" class="@error('skor') border-red-500 @enderror focus:outline-none focus:ring focus:border-blue-300 flex-1 appearance-none text-xs rounded shadow p-3 text-grey-dark mr-2 focus:outline-none">
        ...
        <div class="grid-3 px-2 mt-2 ml-1">
            <div>
                <form action="{{route('importdata')}}" method="POST" enctype="multipart/form-data" class="">
                    {{csrf_field()}}
            ...
            <table class="w-2/5 border">
                ...
                <tbody class="py-5">
                    ...
                    <tr class="bg-gray-100 text-center border-b text-base text-gray-600">
                        ...
                        <td class="p-2 border-r">{{$item -> skor}}</td>
                        <td>
                            <form action="{{route('data.destroy',$item->id)}}" method="POST">
                                @csrf
                                @method('DELETE')
                                <a href="{{route('data.edit',$item->id)}}" class="rounded bg-blue-500 p-2 text-white hover:bg-blue-400 text-xs font-thin">Edit</a>
